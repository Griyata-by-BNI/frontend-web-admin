...
const MOCK_HOUSING_DETAILS: HousingDetails = {
  ...
  propertyTypes: [
    { id: 'tipe-a', name: 'Suvarna Sutera - Tipe A', imageUrl: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=600', price: '819 JT', installment: '8.1jt', bedrooms: 2, buildingArea: 58 },
    { id: 'tipe-b', name: 'Suvarna Sutera - Tipe B', imageUrl: 'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=600', price: '1.4 M', installment: '8.1jt', bedrooms: 3, buildingArea: 80 },
    { id: 'tipe-c', name: 'Suvarna Sutera - Tipe C', imageUrl: 'https://images.unsplash.com/photo-1600566753190-17f0e2723225?w=600', price: '2.1 M', installment: '8.1jt', bedrooms: 3, buildingArea: 110 },
  ],
  facilities: [
    { name: 'Masjid', icon: MasjidIcon }, { name: 'Ruang Komunitas', icon: CommunityIcon },
    { name: 'Taman', icon: ParkIcon }, { name: 'Gym', icon: GymIcon },
  ],
  nearbyLocations: [
    { name: 'SMA N 1 Jakarta', icon: LocationPinIcon }, { name: 'Central Park Mall', icon: LocationPinIcon },
  ],
...
const PropertyTypeCard: React.FC<{ property: PropertyType; developerId: string; clusterId: string; }> = ({ property, developerId, clusterId }) => (
  <Link 
    ...
    <div className="p-4">
      ...
      <div className="flex justify-between items-end mt-3">
        ...
        <div>
          ...
          <p className="text-lg font-bold text-teal-600">Rp {property.installment}<span className="text-xs font-normal">/bln</span></p>
        </div>
      </div>
      <div className="flex text-sm text-gray-600 mt-3 pt-3 border-t">
        <div className="flex items-center mr-4"><BedIcon /><span className="ml-2">KT: {property.bedrooms}</span></div>
        <div className="flex items-center"><BuildingAreaIcon /><span className="ml-2">LB: {property.buildingArea}m²</span></div>
      </div>
    </div>
  </Link>
);
...
export default function HousingDetailPage() {
  ...
  const Map = useMemo(() => dynamic(
    ...
    { ssr: false, loading: () => <div className="flex items-center justify-center h-full bg-gray-200"><p>Loading map...</p></div> }
  ), []);
