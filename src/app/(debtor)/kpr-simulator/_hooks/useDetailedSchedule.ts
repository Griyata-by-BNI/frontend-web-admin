...
import { InterestRate, ScheduleItem } from "../_types";
    ...
    ({
      ...
      if (selectedRate.type === "single-fixed") {
        ...
        for (let month = 1; month <= fixedMonths; month++) {
          ...
          for (const rateInfo of selectedRate.interest_rate as Array<{
            rate: number;
            note: string;
          }>) {
            const match = rateInfo.note.match(
              /tahun ke-(\d+)(?:\s+sampai\s+dengan\s+(\d+))?/
            );
            if (match) {
